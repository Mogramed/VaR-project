...
def ewma_sigma_portfolio(pnl: np.ndarray, lam: float = 0.94) -> np.ndarray:
    """
    Calcule sigma_t (vol) EWMA de la serie pnl (1D).
    Retourne un array sigma (meme taille).
    """
    x = np.asarray(pnl, dtype=float)
    if x.ndim != 1 or len(x) < 5:
        raise ValueError("pnl doit etre 1D et assez long")
    if not (0.0 < lam < 1.0):
        raise ValueError("lam dans (0,1)")

    # init variance avec variance sample des 30 premieres obs (ou moins)
    init_T = min(30, len(x))
    ...
    for t in range(len(x)):
        # update avec observation precedente (standard RiskMetrics-like)
...
def fhs_var_es(
    ...
    - residus z_t = pnl_t / sigma_t
    - VaR/ES sur pertes standardisees, puis re-scale au sigma dernier jour

    Retourne (VaR_loss, ES_loss) en EUR (positif).
    """
    x = np.asarray(pnl_train, dtype=float)
    if len(x) < FHS_MIN_WINDOW:
        raise ValueError("fenetre trop courte pour FHS")
    if not (0.0 < alpha < 1.0):
        raise ValueError("alpha dans (0,1)")

    sigma = ewma_sigma_portfolio(x, lam=lam)
    z = x / sigma
    loss_z = -z  # pertes standardisees (positives)

    var_z = float(np.quantile(loss_z, alpha))
    tail = loss_z[loss_z >= var_z]
    es_z = float(tail.mean()) if len(tail) else var_z

    sigma_next = float(sigma[-1])  # vol "courante" (fin de fenetre)
